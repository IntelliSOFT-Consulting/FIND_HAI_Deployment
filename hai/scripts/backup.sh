...
if [ ${file: -7} == ".sql.gz" ]
  then
    upcount=`$DOCKER_COMPOSE ps cluster_db_1 | grep Up | wc -l`
    if [ $upcount -eq 0 ]
      then
        echo "Starting cluster_db_1 container..."
        $DOCKER_COMPOSE up -d cluster_db_1
    ...
    if [ $started -eq 1 ]
      then
        echo "Stopping cluster_db_1 container..."
        $DOCKER_COMPOSE stop cluster_db_1
    fi
